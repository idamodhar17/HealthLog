...
export const doctorView = async (req, res) => {
  try {
    ...
    const records = await Record.find({ userId: patient._id }).sort({
      createdAt: -1,
    });

    const timeline = await Timeline.find({ userId: patient._id }).sort({
      date: -1,
    });

    const medicalData = await EmergencyProfile.findOne({ userId: patient._id });

    const doctorNotes = await DoctorNote.find({ patientId: patient._id }).sort({
      createdAt: -1,
    });
